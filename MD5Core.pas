{
 ...
 Adapt for Free Pascal Compiler - MARTINEAU Emeric 18/01/2008
...
begin
  ...
  PointerOfCvn := @ctx.Cvn ;

  if cnt > len then begin
     {$IFDEF FPC}
     PointerOfCvn := PointerOfCvn + 64 - cnt ;
     {$ELSE}
     PointerOfCvn := Pointer(Cardinal(PointerOfCvn) + 64 - cnt) ;
     {$ENDIF}

     Move(pb^, PointerOfCvn^, len);
