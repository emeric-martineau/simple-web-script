...
Var { Contient le code à exécuter }
    ...
    { Indique si c'est un return de fonction }
    gbReturn : Boolean = False ;
    { Label courant }
    gsCurrentLabel : String = '' ;
    { Indique si c'est un break de boucle }
    gbBreak : Boolean = False ;
    { Indique si c'est un continue de boucle }
    gbContinue : Boolean = False ;
    {$IFDEF COMMANDLINE}
    { Désactive le support de tag pour la ligne de commande }
    gbTagSupport : Boolean = False ;
    {$ENDIF}
...
Var
    ...
    { Inqique si on est en code exécutable }
    lbCheckIfCodeExecutable : Boolean ;
begin
    lbIsEnd := False ;
    liLine := aiStart ;
    ...
    begin
        ...
        {$IFDEF COMMANDLINE}
        if gbTagSupport = True
        then begin
        {$ENDIF}
            repeat
                lbCheckIfCodeExecutable := True ;
                ...
                then begin
                    ...
                    begin
                        ...
                        OutPutString(aoCode[liLine] + OsEOL() , false) ;
            ...
            until (lbCheckIfCodeExecutable = True) ;
        {$IFDEF COMMANDLINE}
        end ;
        {$ENDIF}
        ...
        { Si c'est un label sur une ligne seul, il est ignoré }
        ...
        then begin
            ...
            then begin
                repeat
                    writeln('') ;
                    writeln('in ' + goListOfFile[goLineToFile[giCurrentLineNumber]]) ;
            ...
            { Au charchement du fichier on élimite le ligne vide. Donc on a
            ...
            { Cherche les labels }
            if (loCurrentLine.Count > 1) and (loCurrentLine[1] = ':')
            then begin
                gsCurrentLabel := loCurrentLine[0] ;

                { Supprime le lable et le : }
                loCurrentLine.Delete(0) ;
                loCurrentLine.Delete(0) ;

                lsCommande := loCurrentLine[0] ;
            end
            else if (Length(gsCurrentLabel) > 0)
            then begin
                gsCurrentLabel := '' ;
            end ;

            {****** COMMANDES ******}
            if lsCommande = 'function'
            then begin
            ...
            then begin
                ...
                then begin
                    ...
                    { S'il y a une erreur on n'execute pas la commande }
                    if (gbQuit = False)
                    then begin
                    ...
                    then begin
                        ExecuteUserProcedure(lsCommande, loCurrentLine) ;
            ...
            if (liLine < aiStart) or (gbQuit = True)
            then begin
                break ;
            end ;
        end ;
